...
-- Set invalid categoryIds to NULL in McqQuestion (only if table exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'McqQuestion') THEN
        UPDATE "McqQuestion"
        SET "categoryId" = NULL
...
-- Add new fields to StudentProfile (only if table and columns don't exist)
DO $$ 
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'StudentProfile') THEN
...
-- Update userStatus column type if it exists as TEXT (only if table exists)
DO $$ 
BEGIN
    -- Only proceed if StudentProfile table exists
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'StudentProfile') THEN
...
-- Add new fields to Project table (only if table exists)
DO $$ 
BEGIN
...
-- Add new fields to Experience table (only if table exists)
DO $$ 
BEGIN
