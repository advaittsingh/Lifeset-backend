...
export class CmsAdminService {
  ...
  async createCurrentAffair(data: any, userId: string) {
    ...
    // Create the post with all fields as columns
    const post = await this.prisma.post.create({
      data: {
        ...
        // Set articleId to post id for MCQ linking
        articleId: undefined, // Will be set after creation
      },
    });

    // Update articleId to post id for MCQ linking
  ...
  async updateCurrentAffair(id: string, data: any) {
    ...
    const updateData: any = {
      ...
      autoGeneratedKeywords: data.autoGeneratedKeywords,
      articleDate: data.articleDate,
      eventDates: data.eventDates !== undefined ? JSON.parse(JSON.stringify(data.eventDates)) : undefined,
      eventYearRange: data.eventYearRange,
      locationLat: data.locationLat,
      locationLong: data.locationLong,
      state: data.state,
  ...
  async updateGeneralKnowledge(id: string, data: any) {
    ...
    const updateData: any = {
      ...
      sendNotification: data.sendNotification,
      notificationTime: data.notificationTime,
      articleType: data.articleType || 'GENERAL_KNOWLEDGE',
    };
    ...
    // Preserve articleId if not provided
    if (!data.articleId) {
      const existingPost = await this.prisma.post.findUnique({
        where: { id },
        select: { articleId: true },
      });
      if (existingPost?.articleId) {
        updateData.articleId = existingPost.articleId;
      } else {
        updateData.articleId = id; // Set to post id if not exists
      }
    }
  ...
  async createMcqQuestion(data: any) {
    ...
    return this.prisma.mcqQuestion.create({
      data: {
        ...
        // MCQ metadata fields (now as columns)
